...
#here I shuffle the patterns that I will use, except the stripe
        ...
        event_table = [
                ...
                Event(3.0+ts1+tsun,         functools.partial(self.show_pattern, pattern_id=spids1[1], gain_x=32, bias_x = 0)),
                Event(3.0+ts1+tsun+tp,      self.do_nothing),

                Event(4.0+ts1+tsun+tp,      functools.partial(self.show_pattern, pattern_id=spids1[1], gain_x=0, bias_x = 0)),
                Event(4.0+ts1+tsun+tp*2,    self.do_nothing),

                Event(5.0+ts1+tsun+tp*2,    functools.partial(self.show_pattern, pattern_id=spids1[2], gain_x=32, bias_x = 0)),
                Event(5.0+ts1+tsun+tp*3,    self.do_nothing),

                Event(6.0+ts1+tsun+tp*3,    functools.partial(self.show_pattern, pattern_id=spids1[2], gain_x=0, bias_x = 0)),
                Event(6.0+ts1+tsun+tp*4,    self.do_nothing),

                Event(7.0+ts1+tsun+tp*4,    functools.partial(self.show_pattern, pattern_id=spids1[1], gain_x=32, bias_x = 0)),
                Event(7.0+ts1+tsun+tp*5,    self.do_nothing),

                Event(8.0+ts1+tsun+tp*5,    functools.partial(self.show_pattern, pattern_id=spids1[1], gain_x=0, bias_x = 0)),
                Event(8.0+ts1+tsun+tp*6,    self.do_nothing),

                Event(9.0+ts1+tsun+tp*6,    functools.partial(self.show_pattern, pattern_id=spids1[2], gain_x=32, bias_x = 0)),
                Event(9.0+ts1+tsun+tp*7,    self.do_nothing),

                Event(10.0+ts1+tsun+tp*7,   functools.partial(self.show_pattern, pattern_id=spids1[2], gain_x=0, bias_x = 0)),
