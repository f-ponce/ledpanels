...
class ExpScript(object):
    ...
    def __init__(self):
        ...
        t2 = 180 #3 min SUN1, no panels
        pattern_id2 = functools.partial(self.panels_off)
        t3 = 0 #dark
        pattern_id3 = functools.partial(self.panels_off)
        t4 = 180 #3 min SUN2, no panels
        pattern_id4 = functools.partial(self.panels_off)
        t5 = 360 #6 min panels moving and SUN2
        pattern_id5 = functools.partial(self.mean_angle_to_pattern, gain_x=spg[0])
        t6 = 180 #3 min SUN2, no panels
        pattern_id6 = functools.partial(self.panels_off)
        t7 = 0 #dark
        pattern_id7 = functools.partial(self.panels_off)
        t8 = 180 #3 min SUN1, no panels
        pattern_id8 = functools.partial(self.panels_off)
        ...
        self.display_action_list = [
                ...
                {'type': 'fixed', 'duration': t4, 'func': pattern_id4},
                {'type': 'mean',  'duration': t5, 'func': pattern_id5},
                {'type': 'fixed', 'duration': t6, 'func': pattern_id6},
                {'type': 'fixed', 'duration': t7, 'func': pattern_id7},
                {'type': 'fixed', 'duration': t8, 'func': pattern_id8},
                ]
